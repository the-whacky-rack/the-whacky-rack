...
(function () {
  ...
  /* ---------- Notification panel element ---------- */
  function getPanel() {
    if (panelEl && document.body.contains(panelEl)) return panelEl;
    panelEl = document.createElement("div");
    panelEl.className = "wr-bell-panel";
  ...
  function positionPanel() {
    const bell = document.querySelector(".wr-bell");
    if (!bell) return;
    const p = getPanel();
    ...
    if (right + panelWidth > window.innerWidth - 10) {
      right = 10;
    }
  ...
  function renderPanel(notifications) {
    ...
    if (!notifications || notifications.length === 0) {
      ...
      bodyHtml = notifications.map(n => {
        ...
        return `
          <div class="wr-notif-row${n.is_read ? "" : " unread"}"
               data-notif-id="${esc(n.id)}"
               data-notif-url="${esc(meta.url)}">
